...
\begin{proposición}
    Sea $\gamma \in \mathcal{C}([0,1], \mathbb{R})$ una curva de Jordan. Entonces la región compacta $A$ limpitada por $\gamma$ se puede dividir en un conjunto finito de regiones de la forma 
...
\end{proposición}
\begin{proof}
    Sea $\gamma = (\gamma_1, \gamma_2)$ y supongamos qsin pérdida de generalidad que $\gamma^{\prime}(t) \neq 0$ para todo $t \in [0,1]$. Así, dado $t \in [0,1]$ tenemos que $\gamma_1(t) \neq 0 $ o $\gamma_2(t) \neq 0$: 
    \begin{enumerate}
        \item $\gamma_1(t) \neq 0$: Entonces, por el teorema de la función inversa, existe un entorno $U$ de $t$ tal que $\gamma_1$ es invertible en $U$, y además, la gráfica e $\gamma$ vendrá dada por una función de clase $\mathcal{C}^1$ de la forma $f(x) = (x, \gamma_2(\gamma_1^{-1}(x)))$, con lo cual existe un entorno de $\gamma(t)$ delimitado por rectas paralelas a los ejes coordenados tal que la intersección de $A$ con dicho entorno es de la primera forma. 
        \item $\gamma_2(t) \neq 0$: Existe un entorno de $\gamma(t)$ delimitado por rectas paralelas a los ejes coordenados tal que la intersección de $A$ con el cuadrado es de la segunda forma.
    \end{enumerate} 
    Sea $t \in A^{\circ}$, tomamos un entorno delimitado por rectas paralelas a los ejes coordenados centrada en $A^{\circ}$. Sea $U$ el recubrimiento formado por los entorno construidas en $A^{\circ}$ y en $\partial A$. \\
    Como $A$ es compacto, existe un subrecubrimiento finito formado por dichos entornos. El conjunto de regiones delimitadas por las rectas que delimitan los entornos, intersecadas con $A$ forman un recubrimiento de $A$ en las condiciones buscadas. 
\end{proof}
...
\begin{proof}
    ...
    \begin{enumerate}
        ...
        \begin{itemize}
            \item $\gamma_1$: de $(a,b)$ a $(c,b)$ (arista inferior)
            \item $\gamma_2$: de $(c,b)$ a $(c,d)$ (arista derecha)
            \item $\gamma_3$: de $(c,d)$ a $(a,d)$ (arista superior)
            \item $\gamma_4$: de $(a,d)$ a $(a,b)$ (arista izquierda)
        \end{itemize}
        ...
        $$\int_{\gamma_1} F \cdot d\gamma = \int_a^c F_1(t, b) dt$$
